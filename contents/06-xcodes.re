...
//image[xcode2][Xcodes][scale=0.7, pos=H]
...
//image[xcode3][Xcodesを起動][scale=0.7, pos=H]

インストール可能なXcodeのバージョンが表示されますので、インストールするバージョンの右側の「INSTALL」ボタンをクリックします。
ダウンロードが完了しましたら、「INSTALL」ボタンが「OPEN」ボタンに変わります。

//image[xcode4][Xcodeのインストール完了][scale=0.7, pos=H]
